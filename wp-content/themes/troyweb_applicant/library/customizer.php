...
function plate_register_theme_customizer( $wp_customize ) {
    ...
    $wp_customize->get_section( 'title_tagline' )->title = __( 'Site Name and Description', 'troywebapplicant' );  
    $wp_customize->get_control( 'blogname' )->label = __( 'Site Name', 'troywebapplicant' );  
    $wp_customize->get_control( 'blogdescription' )->label = __( 'Site Description', 'troywebapplicant' );  
    $wp_customize->get_setting( 'blogname' )->transport = 'postMessage';
    $wp_customize->get_setting( 'blogdescription' )->transport  = 'postMessage';

    // Customize the Front Page Settings
    $wp_customize->get_section( 'static_front_page' )->title = __( 'Homepage Preferences', 'troywebapplicant' );
    $wp_customize->get_section( 'static_front_page' )->priority = 20;
    $wp_customize->get_control( 'show_on_front' )->label = __( 'Choose Homepage Preference:', 'troywebapplicant' );  
    $wp_customize->get_control( 'page_on_front' )->label = __( 'Select Homepage:', 'troywebapplicant' );  
    $wp_customize->get_control( 'page_for_posts' )->label = __( 'Select Blog Homepage:', 'troywebapplicant' );  

    // Customize Background Settings
    $wp_customize->get_section( 'background_image' )->title = __( 'Background Styles', 'troywebapplicant' );  
    $wp_customize->get_control( 'background_color' )->section = 'background_image'; 

    // Customize Header Image Settings  
    $wp_customize->add_section( 'header_text_styles' , array(

        'title'      => __( 'Header Text Styles', 'troywebapplicant' ), 
        'priority'   => 30
